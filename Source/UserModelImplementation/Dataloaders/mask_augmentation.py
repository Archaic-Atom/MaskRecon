...
def debug_main():
    '''
    mask_aug = MaskAug(512, 512, ratio=0.4)
    ...
    left_img = np.array(cv2.imread('/home/lixing/Programs/BodyReconstruction_Mask_2/Example/0000_0_00.jpg'))
    original_x, original_y, height, width = 400, 400, 100, 100
    ...
    cv2.imwrite('/home/lixing/Programs/BodyReconstruction_Mask_2/Example/0006_label.png', gt)
    print(left_img.shape)
    left_img[:, :, 0] = left_img[:, :, 0] * mask + (1 - mask) * left_img[:, :, 0] + np.random.rand(512, 512) * 30 - 15
    left_img[:, :, 1] = left_img[:, :, 1] * mask + (1 - mask) * left_img[:, :, 1] + np.random.rand(512, 512) * 30 - 15
    left_img[:, :, 2] = left_img[:, :, 2] * mask + (1 - mask) * left_img[:, :, 2] + np.random.rand(512, 512) * 30 - 15
    cv2.imwrite('/home/lixing/Programs/BodyReconstruction_Mask_2/Example/crop_0006_reconstruct.png', left_img)

    left_img = np.array(cv2.imread('/home/lixing/Programs/BodyReconstruction_Mask_2/Example/0000_0_00.jpg'))
    left_img[:, :, 0] = left_img[:, :, 0] * (1 - mask) + (mask) * MaskAug.COLOR_GRAY
    left_img[:, :, 1] = left_img[:, :, 1] * (1 - mask) + (mask) * MaskAug.COLOR_GRAY
    left_img[:, :, 2] = left_img[:, :, 2] * (1 - mask) + (mask) * MaskAug.COLOR_GRAY
    cv2.imwrite('/home/lixing/Programs/BodyReconstruction_Mask_2/Example/crop_0006_reconstruct_L.png', left_img)
    ...
    #left_img[:, :, 1] = left_img[:, :, 1] * (1 - mask) + (mask) * MaskAug.COLOR_GRAY
    #left_img[:, :, 2] = left_img[:, :, 2] * (1 - mask) + (mask) * MaskAug.COLOR_GRAY
    #cv2.imwrite('/home/lixing/Programs/BodyReconstruction_Mask_2/Example/color_reconstruct_label.png', left_img)
